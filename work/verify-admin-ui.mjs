...
await page.waitForSelector('.portfolio-manage-button');
await page.getByRole('button', { name: '编辑作品集' }).click();
await page.waitForSelector('#admin-modal[aria-hidden="false"]');
await page.locator('[data-admin-password]').fill('admin');
await page.locator('.admin-login-submit').click();
await page.waitForSelector('[data-admin-workspace]:not([hidden])');
await page.locator('[data-admin-new-folder]').click();
assert.equal(await page.locator('[data-admin-new-folder-form]').getAttribute('hidden'), null);
await page.locator('[data-admin-new-folder-name]').fill('不会创建');
await page.locator('[data-admin-cancel-folder]').click();
assert.notEqual(await page.locator('[data-admin-new-folder-form]').getAttribute('hidden'), null);
